...
fn send_to_appsrc(appsrc: &AppSrc, data: Vec<u8>, mut ts: Duration) -> AnyResult<()> {
    ...
    let buf = {
        // Allocate directly — video frames have nearly unique sizes (entropy coding),
        // so a per-size BufferPool HashMap grows without bound and is the root cause
        // of the long-running memory leak observed with battery cameras.
        let mut gst_buf = gstreamer::Buffer::with_size(data.len())
            .map_err(|e| anyhow!("Failed to allocate GStreamer buffer: {e:?}"))?;
        let gst_buf_mut = gst_buf.get_mut().unwrap();
        let time = ClockTime::from_useconds(ts.as_micros() as u64);
        gst_buf_mut.set_dts(time);
        gst_buf_mut.set_pts(time);
        {
            let mut map = gst_buf_mut.map_writable()
                .map_err(|_| anyhow!("Failed to map GStreamer buffer as writable"))?;
            map.copy_from_slice(data.as_slice());
        }
        gst_buf
    };
